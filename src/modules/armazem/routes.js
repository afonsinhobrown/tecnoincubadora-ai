...
router.post('/pergunta', async (req, res) => {
  ...
  const _lic = await verificarAcesso({ sistemaSlug: 'wms', tenantId: String(req.ctx.farmaciaId || req.ctx.usuarioId), isSuperAdmin: !!req.ctx.isSuperAdmin });
  ...
  try {
    const cacheKey = { sistemaSlug: 'wms', tenantId: String(req.ctx.farmaciaId || req.ctx.usuarioId), query };
  ...
  const { blocos, produtos, modo } = await motor.processar(query, ctx);
    await registrarUsoPrompt({ sistemaSlug: 'wms', tenantId: String(req.ctx.farmaciaId || req.ctx.usuarioId) });
      try { await registrarAuditoria({ sistemaSlug: 'wms', tenantId: String(req.ctx.farmaciaId || req.ctx.usuarioId || ''), tenantNome: _lic.lic?.tenant_nome || '', usuarioId: String(req.ctx.usuarioId||''), usuarioNome: '', query, modo, plano: _lic.plano, licencaStatus: _lic.lic?.status || '', ip: req.ip }); } catch {}
    if (blocos.length || produtos.length) await setCache({ sistemaSlug: 'wms', tenantId: String(req.ctx.farmaciaId || req.ctx.usuarioId), query, resposta: { blocos, produtos, total_produtos: produtos.length, modo } });
