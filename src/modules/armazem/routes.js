...
router.post('/pergunta', async (req, res) => {
  ...
  const { blocos, produtos, modo } = await motor.processar(query, ctx);
    await registrarUsoPrompt({ sistemaSlug: 'wms', tenantId: String(req.ctx.farmaciaId || req.ctx.usuarioId) });
      try { await registrarAuditoria({ sistemaSlug: 'wms' || _lic.plano || 'unknown', tenantId: String(req.ctx.farmaciaId || req.ctx.usuarioId || ''), tenantNome: _lic.lic?.tenant_nome || '', usuarioId: String(req.ctx.usuarioId||''), usuarioNome: '', query, modo, plano: _lic.plano, licencaStatus: _lic.lic?.status || '', ip: req.ip }); } catch {}
