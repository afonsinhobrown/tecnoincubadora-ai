...
<main>
  ...
  <section id="view-chat" style="display:none">
    <div id="sessao-info">
      ...
      <button id="btn-admin" style="display:none; background:#7c3aed; color:#fff; border:0; border-radius:8px; padding:6px 14px; cursor:pointer; font-size:.8rem;" onclick="mostrarAdmin()">👑 Painel Superadmin</button>
  ...
  <!-- Vista Superadmin: gestão de licenças -->
  <section id="view-admin" style="display:none">
    <button class="voltar-link" onclick="mostrarVista('chat')">← Voltar ao chat</button>
    <h2 style="color:var(--azul); margin-bottom:10px;">👑 Subscritores e Licenças</h2>
    <div style="display:flex; gap:8px; margin-bottom:12px; flex-wrap:wrap;">
      <button class="voltar-link" style="background:var(--azul); color:#fff; border-color:var(--azul);" onclick="carregarSubscritores()">↻ Atualizar</button>
      <span id="admin-info" style="font-size:.85rem; color:var(--cinza); align-self:center;"></span>
    </div>
...
const elAdmin = document.getElementById('view-admin');
function mostrarVista(nome) {
  ...
  elAdmin.style.display = nome === 'admin' ? '' : 'none';
}
function mostrarAdmin() { mostrarVista('admin'); carregarSubscritores(); }
async function carregarSubscritores() {
  const token = getToken();
  const el = document.getElementById('admin-lista');
  const info = document.getElementById('admin-info');
  el.innerHTML = 'A carregar...'; info.textContent = '';
  try {
    const r = await fetch('/api/licencas', { headers: { 'Authorization': 'Bearer ' + token } });
    const lista = await r.json();
    if (!r.ok) throw new Error(lista.error || 'Erro');
    if (!lista.length) { el.innerHTML = '<p class="vazio">Nenhum subscritor ainda.</p>'; return; }
    info.textContent = lista.length + ' licenças';
    let html = '<table class="tab"><thead><tr><th>Sistema</th><th>Tenant</th><th>Plano</th><th>Status</th><th>Prompts</th><th>Trial fim</th><th>Ações</th></tr></thead><tbody>';
    lista.forEach(l => {
      const trial = l.trial_fim ? new Date(l.trial_fim).toLocaleDateString('pt-PT') : '—';
      html += \`<tr><td>\${esc(l.sistema_slug)}</td><td>\${esc(l.tenant_nome||l.tenant_id)}</td><td>\${esc(l.plano)}</td><td>\${esc(l.status)}</td><td>\${l.prompts_usados}/\${l.plano==='pro'?'∞':(l.plano==='standard'?80:50)}</td><td>\${trial}</td><td style="white-space:nowrap"><button onclick="acaoLicenca('\${l.sistema_slug}','\${l.tenant_id}','renovar')" style="font-size:.75rem; padding:4px 8px; border:1px solid var(--borda); border-radius:6px; cursor:pointer;">Renovar +10d</button> <button onclick="acaoLicenca('\${l.sistema_slug}','\${l.tenant_id}','bloquear')" style="font-size:.75rem; padding:4px 8px; border:1px solid #fca5a5; border-radius:6px; cursor:pointer; color:#991b1b;">\${l.status==='blocked'?'Desbloquear':'Bloquear'}</button> <button onclick="acaoLicenca('\${l.sistema_slug}','\${l.tenant_id}','plano')" style="font-size:.75rem; padding:4px 8px; border:1px solid var(--borda); border-radius:6px; cursor:pointer;">Plano</button></td></tr>\`;
    });
    html += '</tbody></table>';
    el.innerHTML = html;
  } catch(e) { el.innerHTML = '<p class="alerta">Erro: ' + esc(e.message) + '</p>'; }
}
async function acaoLicenca(sistema, tenantId, acao) {
  const token = getToken();
  try {
    let url = '', method = 'POST', body = {};
    if (acao === 'renovar') { url = \`/api/licencas/\${sistema}/\${tenantId}/renovar\`; body = { dias: 10 }; }
    else if (acao === 'bloquear') {
      const r = await fetch('/api/licencas', { headers: { 'Authorization': 'Bearer ' + token } });
      const lista = await r.json();
      const lic = lista.find(x=> x.sistema_slug===sistema && x.tenant_id===tenantId);
      const bloquear = lic?.status !== 'blocked';
      url = \`/api/licencas/\${sistema}/\${tenantId}/\${bloquear?'bloquear':'desbloquear'}\`; body = {};
    }
    else if (acao === 'plano') {
      const novo = prompt('Novo plano (basico, standard, pro):');
      if (!novo || !['basico','standard','pro'].includes(novo)) return;
      url = \`/api/licencas/\${sistema}/\${tenantId}\`; method = 'PATCH'; body = { plano: novo };
    }
...
function arraysDaResposta() {
  ...
  (ULTIMA_RESPOSTA?.blocos || []).forEach(b => {
    const d = b.dados;
    if (Array.isArray(d)) { if (d.length && typeof d[0] === 'object') arr.push({ titulo: b.titulo, linhas: d }); }
    else if (d && typeof d === 'object') {
      if (Array.isArray(d.lista)) arr.push({ titulo: b.titulo + ' (lista)', linhas: d.lista });
      if (Array.isArray(d.por_forma_pagamento)) arr.push({ titulo: b.titulo + ' (por forma)', linhas: d.por_forma_pagamento });
      for (const chave of Object.keys(d)) {
        if (['lista', 'por_forma_pagamento'].includes(chave)) continue;
        const v = d[chave];
        if (Array.isArray(v) && v.length && typeof v[0] === 'object') arr.push({ titulo: b.titulo + ' (' + chave.replace(/_/g, ' ') + ')', linhas: v });
      }
    }
  });
  return arr;
...
function iniciarChat(d) {
  ...
  if (s.slug === 'ddgei') elSessaoNome.textContent = 'DDGEI';
  document.getElementById('btn-admin').style.display = (d.usuario && d.usuario.tipo_usuario === 'SUPERADMIN') ? '' : 'none';
...
function renderBloco(b) {
  ...
  acoes.style.cssText = 'display:flex;gap:6px;margin-top:12px;flex-wrap:wrap;';
  const novoBtn = (txt, fn, bg='var(--azul-claro)', cor='var(--azul)') => {
    const bt = document.createElement('button');
    bt.textContent = txt; bt.style.cssText = \`background:\${bg};color:\${cor};border:0;border-radius:999px;padding:6px 12px;font-size:.8rem;cursor:pointer;\`;
    bt.onclick = fn; return bt;
  };
  const acoesLike = novoBtn('👍 Gostei', () => darFeedback(b, true));
  const acoesDislike = novoBtn('👎 Não gostei', () => darFeedback(b, false), '#fee2e2', '#991b1b');
  const acoesCopiar = novoBtn('📋 Copiar', () => copiarBloco(b));
  const acoesShare = novoBtn('📤 Partilhar', () => partilharBloco(b), '#dcfce7', '#166534');
  const btnExp = novoBtn('🔍 Explorar mais', () => explorarMais(b.titulo, b.intencao));
  const btnDet = novoBtn('➕ Adicionar detalhes', () => adicionarDetalhes(b.titulo), '#fff', 'var(--azul)');
  acoes.append(acoesLike, acoesDislike, acoesCopiar, acoesShare, btnExp, btnDet);
  el.append(acoes);
...
function textoDoBloco(b) {
  ...
  const d = b.dados;
  const ser = v => String(v ?? '').replace(/\\s+/g, ' ').trim();
  if (Array.isArray(d)) { d.slice(0, 30).forEach(r => linhas.push('- ' + Object.values(r).map(ser).join(' | '))); }
  else if (d && typeof d === 'object') {
    if (d.totais && typeof d.totais === 'object') Object.entries(d.totais).filter(([,v])=>typeof v!=='object').forEach(([k,v])=>linhas.push('• '+k.replace(/_/g,' ')+': '+v));
    if (d.vencedor && typeof d.vencedor === 'object') linhas.push('🏆 Vencedor: '+ (d.vencedor.partido||'') + (d.vencedor.votos? ' — '+Number(d.vencedor.votos).toLocaleString('pt-MZ')+' votos':'') + (d.vencedor.pct? ' ('+d.vencedor.pct+'%)':''));
    const lista = d.lista || d.por_forma_pagamento;
    if (Array.isArray(lista)) lista.slice(0,30).forEach(r => linhas.push('- '+Object.values(r).map(ser).join(' | ')));
    for (const chave of Object.keys(d)) {
      if (['lista','por_forma_pagamento'].includes(chave)) continue;
      const arr = d[chave];
      if (Array.isArray(arr) && arr.length && typeof arr[0]==='object')
        arr.slice(0,30).forEach(r => linhas.push('- '+Object.values(r).map(ser).join(' | ')));
    }
  } else if (b.texto) linhas.push(b.texto);
  return linhas.join('\\n');
}
function copiarBloco(b) {
  const t = 'TECNOINCUBADORA AI · ' + (getSistema()?.nome||'') + '\\n' + textoDoBloco(b);
  if (navigator.clipboard) navigator.clipboard.writeText(t).then(()=>alert('Copiado!')).catch(()=>fallbackCopy(t));
  else fallbackCopy(t);
}
function fallbackCopy(t) { const ta=document.createElement('textarea'); ta.value=t; document.body.appendChild(ta); ta.select(); document.execCommand('copy'); ta.remove(); alert('Copiado!'); }
function partilharBloco(b) {
  const t = textoDoBloco(b);
  window.open('https://wa.me/?text=' + encodeURIComponent('TECNOINCUBADORA AI · ' + (getSistema()?.nome||'') + '\\n' + t), '_blank');
}
async function darFeedback(b, gostou) {
  const token = getToken();
  if (!token) { alert('Inicie sessão.'); return; }
  let comentario = '';
  if (!gostou) comentario = prompt('Que sugestão/comentário?', '') || '';
  try {
    const r = await fetch('/api/feedback', {
      method: 'POST', headers: { 'Content-Type':'application/json', 'Authorization':'Bearer '+token },
      body: JSON.stringify({ sistemaSlug: getSistema()?.slug, query: ULTIMA_RESPOSTA?.query || '', blocoTitulo: b.titulo, blocoIntencao: b.intencao, gostou, comentario })
    });
    const d = await r.json();
    if (!r.ok) throw new Error(d.error || 'Erro');
    alert(gostou ? 'Obrigado pelo feedback! 👍' : 'Feedback registado. Obrigado!');
  } catch(e) { alert('Erro: '+e.message); }
}
...
// rótulo de âmbito (ano + espaço territorial) sempre visível nos resultados
function escopoResultado(f) {
  if (!f || typeof f !== 'object') return '';
  const bits = [];
  const tipo = f.tipo ? String(f.tipo).trim() : '';
  if (tipo) bits.push(cap(tipo));
  if (f.ano) bits.push('Ano ' + f.ano);
  const niveis = [
    ['localidade', 'Localidade de'],
    ['posto', 'Posto administrativo de'],
    ['distrito', 'Distrito de'],
    ['provincia', 'Província de']
  ];
  let temGeo = false;
  for (const [k, pre] of niveis) {
    if (f[k]) { bits.push('📍 ' + pre + ' ' + String(f[k])); temGeo = true; break; }
  }
  if (!temGeo && f.agrupar) bits.push('📍 agrupado por ' + cap(String(f.agrupar).replace(/_/g, ' ')));
  if (!bits.length) bits.push('Nacional');
  return bits.join(' · ');
}

// rótulos amigáveis e formatação para tabelas de resultados eleitorais
const LABEL_STATSE = {
  partido: 'Partido', votos: 'Votos', pct: '%',
  percentual_validos: '% votos válidos', percentual_inscritos: '% inscritos',
  pct_anterior: '% anterior', votos_anterior: 'Votos anterior',
  delta_pct: 'Variação %', delta_votos: 'Variação votos',
  diferenca_votos: 'Diferença votos', sentido: 'Sentido',
  zona: 'Zona', total_votos: 'Total votos', vencedor: 'Vencedor'
};
function valTabelaStatse(k, v) {
  if (v == null || v === '') return '—';
  if (typeof v === 'number') {
    if (['votos', 'votos_anterior', 'diferenca_votos', 'delta_votos', 'total_votos'].includes(k)) return Number(v).toLocaleString('pt-MZ');
    if (['delta_pct', 'delta_votos'].includes(k)) return (v >= 0 ? '+' : '') + v;
    return String(v);
  }
  return String(v);
}
...
function renderPartidosChart(container, lista, foco, escopo) {
  ...
  tit.textContent = '📊 Resultado por partido' + (escopo ? ' · ' + escopo : '') + (temDelta ? ' (▲ subiu · ▼ caiu vs. anterior)' : '');
  ...
  lista.forEach(x => {
    ...
    trilho.style.cssText = 'flex:1;background:var(--borda);border-radius:6px;height:20px;overflow:hidden;box-shadow:' + (ehFoco ? '0 0 0 2px #111827' : 'none') + ';';
...
// Gráfico que ilustra as tendências: barra cinzenta = ano anterior, barra a cor = ano atual
function renderTendenciaChart(container, ev, anoAnt, foco) {
  let max = 1;
  ev.forEach(x => { const v = Math.max(Number(x.pct) || 0, Number(x.pct_anterior) || 0); if (v > max) max = v; });
  const fn = (s) => String(s || '').toUpperCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '');
  const focoN = fn(foco);
  const wrap = document.createElement('div');
  ...
  if (anoAnt) {
    ...
    leg.style.cssText = 'font-size:.7rem;color:var(--cinza);margin-bottom:6px;';
    leg.textContent = '■ ' + anoAnt + ' · ■ atual (' + String(ev[0]?.ano ?? '') + ')';
    wrap.appendChild(leg);
  }
  ev.forEach(x => {
    const ant = Number(x.pct_anterior) || 0;
    const atu = Number(x.pct) || 0;
    const ehFoco = focoN && fn(x.partido) === focoN;
    const linha = document.createElement('div');
    linha.style.cssText = 'display:flex;align-items:center;margin:3px 0;gap:6px;' + (ehFoco ? 'background:' + corPartido(x.partido) + '1f;border-radius:6px;' : '');
    const lbl = document.createElement('div');
    lbl.style.cssText = 'width:96px;font-size:.72rem;text-align:right;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;color:var(--cinza);font-weight:' + (ehFoco ? '700' : 'normal') + ';';
    lbl.textContent = (ehFoco ? '▶ ' : '') + x.partido;
    const trilho = document.createElement('div');
    trilho.style.cssText = 'flex:1;display:flex;gap:2px;height:14px;';
    const bAnt = document.createElement('div');
    bAnt.style.cssText = 'width:' + Math.max(0, Math.round((ant / max) * 100)) + '%;background:#cbd5e1;border-radius:3px;';
    const bAtu = document.createElement('div');
    bAtu.style.cssText = 'width:' + Math.max(0, Math.round((atu / max) * 100)) + '%;background:' + corPartido(x.partido) + ';border-radius:3px;';
    trilho.append(bAnt, bAtu);
    const val = document.createElement('div');
    val.style.cssText = 'width:160px;font-size:.7rem;white-space:nowrap;color:' + (ehFoco ? 'var(--azul)' : 'var(--cinza)') + ';font-weight:' + (ehFoco ? '700' : 'normal') + ';';
    let seta = '';
    if (x.delta_pct != null) seta = (x.delta_pct > 0 ? ' ▲' : x.delta_pct < 0 ? ' ▼' : ' •') + (x.delta_pct > 0 ? '+' : '') + x.delta_pct + 'pts';
    val.textContent = ant + '% → ' + atu + '%' + seta;
    linha.append(lbl, trilho, val);
    wrap.appendChild(linha);
  });
  container.appendChild(wrap);
...
function renderDados(container, dados) {
  ...
  if (dados && typeof dados === 'object') {
    const kpiHtml = (entradas) => entradas.map(function (par) {
      const k = par[0], v = par[1];
      return '<div class="kpi"><div class="valor">' + esc(String(v)) + '</div><div class="rotulo">' + esc(cap(String(k).replace(/_/g, ' '))) + '</div></div>';
    }).join('');
    if (dados.totais && typeof dados.totais === 'object') {
      const entradas = Object.entries(dados.totais).filter(([, v]) => v != null && typeof v !== 'object');
    ...
    const escopo = escopoResultado(dados.filtro);
    ...
    if (temResultEleitoral) {
      ...
      faixa.textContent = '🗺️ Âmbito: ' + (escopo || 'Nacional') + (dados.foco ? ' · Foco: ' + dados.foco : '');
      container.appendChild(faixa);
    }
    // gráfico colorido por partido (adversários no mesmo gráfico), destacando o foco
    if (Array.isArray(dados.partidos) && dados.partidos.length) renderPartidosChart(container, dados.partidos, dados.foco, escopo);
    // realça um eventual vencedor (ex: resultados eleitorais)
    if (dados.vencedor && typeof dados.vencedor === 'object') {
      const v = dados.vencedor;
      let txt = '<div style="margin:10px 0"><span style="font-size:.9rem">🏆 Vencedor: <b>' + esc(String(v.partido ?? v.nome ?? '')) + '</b>';
      if (v.votos) txt += ' — ' + Number(v.votos).toLocaleString('pt-MZ') + ' votos';
      if (v.pct) txt += ' (' + v.pct + '%)';
      txt += '</span></div>';
    ...
    if (dados.tendencia && Array.isArray(dados.tendencia.evolucao) && dados.tendencia.evolucao.length) {
      ...
      const anoAnt = dados.tendencia.ano_anterior ?? 'anterior';
      const h = document.createElement('div');
      h.style.cssText = 'font-size:.85rem;font-weight:600;margin:12px 0 4px;';
      h.textContent = '📈 Tendência por partido' + (escopoResultado(dados.filtro) ? ' · ' + escopoResultado(dados.filtro) : '') + ' vs ' + anoAnt;
      container.appendChild(h);
      renderTendenciaChart(container, ev, anoAnt, dados.foco);
      const nota = document.createElement('div');
      nota.style.cssText = 'font-size:.72rem;color:var(--cinza);margin:2px 0 6px;';
      nota.textContent = 'Como ler: “Variação % (Delta pct)” = pontos percentuais de diferença; “Variação votos (Delta votos)” = diferença de votos. ▲ subiu · ▼ caiu vs ' + anoAnt + '.';
    ...
    // recomendações em tabela
    if (dados.recomendacoes && Array.isArray(dados.recomendacoes.lista) && dados.recomendacoes.lista.length) {
      const rc = dados.recomendacoes;
      const hr = document.createElement('div');
      hr.style.cssText = 'font-size:.85rem;font-weight:600;margin:12px 0 4px;';
      hr.textContent = rc.titulo || '✅ Recomendações';
      container.appendChild(hr);
      renderTabela(container, rc.lista, ['Tema', 'Recomendação'],
        r => [esc(String(r.tema || '')), esc(String(r.sugestao || ''))], false, null, r => corLinhaFoco(r, dados.foco));
    }
    // qualquer array de topo vira tabela (ex: partidos_analise, rivais_proximos, por_provincia)
    for (const chave of Object.keys(dados)) {
      if (chave === 'partidos' || chave === 'evolucao') continue; // já ilustrados no gráfico/tendência
      const arr = dados[chave];
      if (Array.isArray(arr) && arr.length && typeof arr[0] === 'object') {
        const DROP = chave === 'partidos_analise' ? ['pct_anterior', 'votos_anterior', 'delta_pct', 'delta_votos'] : [];
        ...
        h.textContent = cap(String(chave).replace(/_/g, ' ')) + (ehEleitoral && escopo ? ' — ' + escopo : '');
        container.appendChild(h);
        const etiquetas = chaves.map(c => LABEL_STATSE[c] || cap(String(c).replace(/_/g, ' ')));
        renderTabela(container, arr, etiquetas,
          r => chaves.map(c => valTabelaStatse(c, r[c])), false, null, r => corLinhaFoco(r, dados.foco));
      }
    }
  }
}
